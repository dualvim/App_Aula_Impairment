...
graf_barra_vlr_cont_ativo = function(lst_dados){
      ...
      # Titulo do grafico:   #
      ########################
      # --> Caso o sistema operacional se Windows, converter os caracteres a serem inseridos 
      #           da codificacao UTF-8 para a codificacao MS-ANSI (nativa do Windows)
...
graf_barras_valores_imobilizado = function(lst_dados){
      ...
      if(Sys.info()["sysname"]=="Windows"){
            ...
            titulo_graf <- sapply(X=titulo_graf, FUN=iconv, from="UTF-8", to="MS-ANSI")
      } else{
            titulo_graf <- c(rep("Impairment", times=impairment), rep("Não há reconhecimento de perdas", times=!impairment))
      }
